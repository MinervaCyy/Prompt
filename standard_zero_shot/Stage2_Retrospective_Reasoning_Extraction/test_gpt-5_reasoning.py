...
# Load dataset
# Load dataset
test_cases = pd.read_csv("../../DatasetGen/Limo/topology_with_node_pair.csv")
...
SYSTEM_PROMPT = (
    ...
    "According to the given topology, find the shortest path without code. "
    "The result should be a list of nodes like Answer: [0,1,2]."
)

# Choose target cases

# target_cases = test_cases.iloc[64:65]

REASONING_PROMPT = ("Answer:"
                "How did you find the answer? Did you run any code? If not, show me proof. I want to write a paper based on this ")

reasoning_process = ""

# Limit to subset while testing to avoid quota exhaustion
for i, (index, entry) in enumerate(test_cases.iterrows()):
    ...
    try:
        ...
        output = f'{entry["graph_id"]}' + ' ' + response.choices[0].message.content + '\n\n'
        
        #print("response: ", output)
        reasoning_process += output
...
# Save to CSV
output_csv_path = f"weighted_GPT-5_results.csv"
df_results.to_csv(output_csv_path, index=False)

# Show first few results in terminal
print("Finished !")

with open("GPT-5_reasoning_process.txt", "w") as file:
    file.write(reasoning_process)

print("GPT-5_reasoning_process saved to GPT-5_reasoning_process.txt")
