...
# Load dataset
# Load dataset
test_cases = pd.read_csv("../topology_with_node_pair.csv")
...
SYSTEM_PROMPT = (
        ...
        "According to the given topology, find the shortest path. "
        "Topology: {0: [(1, 15), (2, 45)], 1: [(0, 15), (3, 20), (4, 50)], 2: [(0, 45), (5, 10), (6, 30)], 3: [(1, 20)], 4: [(1, 50)], 5: [(2, 10)], 6: [(2, 30)]}\n"
        "Node pair: [3, 6]\n"
        "Answer: [3, 1, 0, 2, 6]\n\n"
        ...
        "Let's think step-by-step. First, analyse the adjacent nodes and edge weights, then trace the optimal path. "
        "Conclude your response with your final answer formatted exactly as Answer: [0, 1, 2]."
    )
...
target_cases = pd.concat([test_cases.iloc[1870:1940], test_cases.iloc[2850:2870]])
# Limit to subset while testing to avoid quota exhaustion
for i, (index, entry) in enumerate(target_cases[:5].iterrows()):
...
# Save to CSV
output_csv_path = f"weighted_GPT-4o_cot_oneshot_results.csv"
